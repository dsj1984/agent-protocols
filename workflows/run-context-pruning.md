...
1.  Identify the location of `[DOCS_ROOT]/decisions.md` (ADRs),
    `[DOCS_ROOT]/patterns.md`, and any other core context documentation
    mentioned in `instructions.md`.
2.  Ensure a directory `[DOCS_ROOT]/archive/` exists; if not, create it.

## Step 1 - Content Analysis & Archiving

1. **Analyze Documentation**: Review `[DOCS_ROOT]/decisions.md`,
   `[DOCS_ROOT]/patterns.md`, and project-wide rules for entries marked as
   `Superseded`, `Deprecated`, `Obsolete`, or those that directly contradict the
...
3. **Archive Records**: Append the extracted blocks to
   `[DOCS_ROOT]/archive/deprecated-decisions.md` or
   `[DOCS_ROOT]/archive/deprecated-patterns.md` respectively.
...
1. **Rebuild Context Index**: Trigger the context indexer to reflect the
   changes: `node [SCRIPTS_ROOT]/context-indexer.js index`
...
it to `[DOCS_ROOT]/archive/` to ensure it remains available for manual lookup
while being invisible to the AI-native Local RAG.
