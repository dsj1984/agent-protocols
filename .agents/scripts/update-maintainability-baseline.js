import { scanDirectory, calculateAll, saveBaseline } from "./lib/maintainability-utils.js";
...
async function main() {
  ...
  console.log(`[Maintainability] Calculating scores for ${files.length} files...`);
  const scores = calculateAll(files);

  saveBaseline(scores);
  
  console.log("[Maintainability] ✅ Baseline updated successfully in .agents/maintainability-baseline.json");
}
