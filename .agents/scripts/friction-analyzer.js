...
async function main() {
  ...
  if (!epicId) {
    Logger.info(
      '[FrictionAnalyzer] No --epic provided. Attempting to find recent closed Epics...',
    );
    const epics = await provider.getEpics();
    const closedEpics = epics
      .filter((e) => e.state === 'closed')
      .sort((a, b) => b.id - a.id);
    if (closedEpics.length > 0) {
      epicId = closedEpics[0].id;
      Logger.info(
        `[FrictionAnalyzer] Using most recently closed Epic #${epicId}.`,
      );
    } else {
      Logger.error(
        '[FrictionAnalyzer] Error: --epic <id> is required, and no closed Epics were found.',
      );
      process.exit(1);
    }
  }
  ...
  if (logs.length === 0) {
    Logger.info(
      `[FrictionAnalyzer] No friction logs found for Epic #${epicId}. Exiting.`,
    );
    return;
  }
  ...
  if (patterns.length === 0) {
    Logger.info(
      `[FrictionAnalyzer] No actionable patterns identified. Exiting.`,
    );
    return;
  }
  ...
  for (const pattern of topPatterns) {
    if (!pattern.protocolFile) {
      Logger.warn(
        `[FrictionAnalyzer] Skipping pattern "${pattern.category}" because it lacks a specific protocolFile.`,
      );
      continue;
    }

    Logger.info(
      `[FrictionAnalyzer] Processing pattern: ${pattern.category} in ${pattern.protocolFile} (${pattern.eventCount} events)`,
    );
    ...
    try {
      ...
      if (!fs.existsSync(absPath)) {
        Logger.warn(
          `[FrictionAnalyzer] File ${pattern.protocolFile} not found.`,
        );
        continue;
      }
      ...
      const suggestion = await refinementAgent.generateSuggestion(
        pattern,
        currentContent,
      );
      ...
      if (!suggestion?.newContent) {
        Logger.warn(
          `[FrictionAnalyzer] LLM did not return a valid suggestion.`,
        );
        continue;
      }
      ...
      if (suggestion.newContent.trim() === currentContent.trim()) {
        Logger.info(
          `[FrictionAnalyzer] LLM returned identical content. No refinement needed.`,
        );
        continue;
      }

      const pr = await githubService.proposeRefinement(
        pattern,
      ...
      if (pr) {
        Logger.info(
          `[FrictionAnalyzer] Successfully proposed refinement PR #${pr.number}: ${pr.url}`,
        );
      }
    } catch (err) {
      Logger.error(
        `[FrictionAnalyzer] Error processing pattern ${pattern.category}: ${err.message}`,
      );
    }
  }
