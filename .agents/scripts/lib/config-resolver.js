...
// Auto-load .env from the workspace root if it exists
// This avoids having to call dotenv or --env-file for every script.
try {
...
export function resolveConfig(opts) {
  ...
  if (fs.existsSync(agentrcPath)) {
    ...
    for (const key of schemaValidKeys) {
      if (typeof settings[key] === 'string' && SHELL_INJECTION_RE.test(settings[key])) {
        throw new Error(
...
export function validateOrchestrationConfig(orchestration) {
  ...
  if (orchestration.provider === 'github' && orchestration.github) {
    ...
    if (typeof gh.owner === 'string' && SHELL_INJECTION_RE.test(gh.owner)) {
      errors.push('[Security] Shell meta-characters detected in orchestration.github.owner.');
    }
    if (typeof gh.repo === 'string' && SHELL_INJECTION_RE.test(gh.repo)) {
      errors.push('[Security] Shell meta-characters detected in orchestration.github.repo.');
    }
    if (typeof gh.operatorHandle === 'string' && SHELL_INJECTION_RE.test(gh.operatorHandle)) {
      errors.push('[Security] Shell meta-characters detected in orchestration.github.operatorHandle.');
    }
  }

  // Notification webhook injection check
  if (orchestration.notifications?.webhookUrl) {
    if (typeof orchestration.notifications.webhookUrl === 'string' &&
        SHELL_INJECTION_RE.test(orchestration.notifications.webhookUrl)) {
      errors.push('[Security] Shell meta-characters detected in orchestration.notifications.webhookUrl.');
    }
  }
