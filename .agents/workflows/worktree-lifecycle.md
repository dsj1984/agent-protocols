---
description: >-
  Per-story git worktree isolation model — configuration, lifecycle, node_modules
  strategies, Windows notes, fallback mode, and human-reviewer guidance.
---
...
`git worktree` at `.worktrees/story-<id>/` so branch swaps, staging, and
reflog activity are isolated per-story. The main checkout stays quiet.
...
The schema is validated by `config-resolver.js`. Unknown strategies,
`root` values that escape the repo root, and shell-metacharacter injection in
`root` are all rejected at config-load time.
...
authority for `ensure`, `reap`, `list`, `isSafeToRemove`, and `gc`. No other
script may call `git worktree` directly.
...
- On Windows, `allowSymlinkOnWindows: true` is required — symlink semantics
  vary by Windows version and may demand admin rights.
...
- **Long-path warning**: when `worktreePath.length + 80` exceeds
  ...
  Relocate `orchestration.worktreeIsolation.root` to a shorter prefix
  (e.g. `C:\w`) if you see this.
...
- `assert-branch.js` and `computeStoryWaves` focus-area serialization remain
  in place as the primary race guards.
...
- The Epic branch accumulates the cumulative diff for code review; that lives
  on the main checkout, not in any per-story worktree.
- Opening a worktree in an IDE can mislead: the working directory looks like
  the main repo but carries a different HEAD. The main checkout is the
  canonical place to read PRDs, Tech Specs, and run `/sprint-code-review`.
...
- **Force-remove a worktree**: the framework **never** passes `--force` to
  ...
  operators can manually run `git worktree remove --force <path>`. Confirm
  there is no uncommitted work first.
