...
# /sprint-execute-story #[Story ID]

## Overview

`/sprint-execute-story` is a **single-purpose worker**. One invocation runs one Story
...
`/sprint-execute-story <Story ID>` per Claude window; the operator owns launch order
...
`/sprint-execute-story <id>` in each. With `worktreeIsolation.enabled: true` each
