...
// The overlay filename is module-local to the helper, so it is restated here
// deliberately: the test asserts the sweep reaches this exact path.
const LOCAL_PATH = path.join(PROJECT_ROOT, '.agentrc.local.json');
...
describe('createRetireAgentrcKeyStep — surface', () => {
  ...
  it('defaults to sweeping both config surfaces', () => {
    // Asserted behaviourally rather than against the constant: a step that
    // declares no `filenames` must still reach the overlay.
    const { ctx, read } = makeCtx({
      base: { planning: { complexityGate: { enabled: true } } },
      local: { planning: { complexityGate: { retired: 1 } } },
    });
