 ...
 *
 * `pin-current` fails when the version actually resolvable in
 * `node_modules` is inconsistent with the consumer's declared `mandrel`
 * dependency *range* (`package.json`) — the skew #4525 reported doctor
 * grading as fully green. It is distinct from (and, unlike)
 * `version-current`: that check stays a non-fatal, cache-only "is a newer
 * version published?" advisory; this one is fatal, and answers "does the
 * declared dependency describe what is actually installed?".
 *
 * The pin is a **range**, so satisfaction — not identity — is the graded
 * question: `^2.1.0` + an installed 2.4.0 is a healthy `npm update`, not a
 * defect, and passes with an advisory `detail`. Only genuinely broken
 * states fail: installed outside the declared range, or behind it.
 *
...
describe('runPinCurrent — installed version is inconsistent with the declared range', () => {
  it('fails and names both versions when the installed version is outside the declared range (the #4525 repro)', () => {
    ...
    assert.match(result.detail, /\^1\.87\.0/);
    assert.match(result.detail, /v2\.0\.0/);
    assert.match(result.remedy, /mandrel update/);
  });

  it('fails when an exact pin disagrees with the installed version (no range to satisfy)', () => {
    const fsImpl = makeFsFake({
      [path.join(CONSUMER_ROOT, 'package.json')]: pkgJson({
        mandrel: '2.1.0',
      }),
    ...
    assert.equal(result.ok, false);
    assert.match(result.detail, /outside the declared range/);
  });

  it('fails when a tilde pin is exceeded on the minor axis', () => {
    const fsImpl = makeFsFake({
      [path.join(CONSUMER_ROOT, 'package.json')]: pkgJson({
        mandrel: '~2.1.0',
      }),
  ...
  it('fails when the pin is AHEAD of what is installed, with an npm install remedy', () => {
    const fsImpl = makeFsFake({
      [path.join(CONSUMER_ROOT, 'package.json')]: pkgJson({
        mandrel: '^2.0.0',
      }),
...
// ---------------------------------------------------------------------------
// In-range-newer — healthy, NOT fatal (the A13 regression this check caused)
// ---------------------------------------------------------------------------

describe('runPinCurrent — an in-range-newer install is healthy, not a failure', () => {
  it('passes with an advisory when a caret pin is satisfied by a newer install (the `npm update` steady state)', () => {
    const fsImpl = makeFsFake({
      [path.join(CONSUMER_ROOT, 'package.json')]: pkgJson({
        mandrel: '^2.1.0',
      }),
    ...
    assert.match(result.detail, /advisory/);
    assert.match(result.detail, /mandrel update/);
    assert.match(result.detail, /v2\.4\.0/);
  });

  it('passes with an advisory when a tilde pin is satisfied by a newer patch', () => {
    const fsImpl = makeFsFake({
      [path.join(CONSUMER_ROOT, 'package.json')]: pkgJson({
        mandrel: '~2.1.0',
      }),
...
describe('runPinCurrent — pin matches installed', () => {
  it('passes when the pin base version equals the installed version', () => {
    ...
    assert.doesNotMatch(result.detail, /advisory/);
  });

  it('passes quietly when an exact pin equals the installed version', () => {
    const fsImpl = makeFsFake({
      [path.join(CONSUMER_ROOT, 'package.json')]: pkgJson({
        mandrel: '2.1.0',
      }),
