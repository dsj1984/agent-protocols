...
describe('runSkillSmoke', () => {
  it('returns pass=true for the gates-and-baselines skill (real, on-disk)', async () => {
    const result = await runSkillSmoke({
      skillName: 'gates-and-baselines',
    });
    assert.equal(
      result.pass,
      true,
    ...
    assert.equal(result.skill.name, 'gates-and-baselines');
  ...
  it('invokes the validator with the parsed skill + fixture', async () => {
    ...
    assert.equal(received.skill.name, 'gates-and-baselines');
