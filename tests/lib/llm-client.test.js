...
test('LLMClient: _decodeHtmlEntities', () => {
    const client = new LLMClient({ orchestration: { llm: { provider: 'openai' } } });
    const input = 'a &lt; b &amp; c &gt; d';
...
test('LLMClient: generateText retry logic (Gemini mock)', async () => {
    ...
    global.fetch = async () => {
        attempts++;
        if (attempts === 1) {
            return { ok: false, status: 429, statusText: 'Too Many Requests', text: async () => 'Rate limit' };
        }
        return {
            ok: true,
            json: async () => ({
...
test('LLMClient: generateText (Anthropic mock)', async () => {
    ...
    const client = new LLMClient({ orchestration: { llm: { provider: 'anthropic' } } });
