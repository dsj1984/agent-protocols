 ...
 * expects); `createStory` / `createPlanSeed` model the `/mandrel-plan` and
 * `/mandrel-plan --idea` surfaces by handing back a fresh issue record.
 */
...
test('promoteFindings: new tight cluster opens a Story via /mandrel-plan port', async () => {
...
test('promoteFindings: broad cluster opens a plan-seed via /mandrel-plan --idea port', async () => {
  const items = [
...
test('promoteFindings: runs offline through injected ports (AC #3)', async () => {
  ...
  const result = await promoteFindings(items, {
    ...
    createStory: async (cluster) => {
      assert.ok(cluster.title, 'cluster carries a title for /mandrel-plan');
