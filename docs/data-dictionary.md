...
| `sprint-hierarchy-gate.js`                          | Script                | `/sprint-close` Phase 1.2 gate; walks the Epic's full sub-issue graph (Features → Stories → Tasks plus auxiliary tickets) and exits non-zero if any descendant is open or any Task is closed without `agent::done`. Pairs with `sprint-wave-gate.js` (manifest view) for the Phase 1 Feature Completeness Check. |
